...
class AutoSuggestionEngine
{
    ...
    public function suggestFromCode(string $code, int $limit = 8): array
    {
        ...
        if ((str_contains($code, 'cache()->') || str_contains($code, 'CacheInterface') || str_contains($code, 'FileCache') || str_contains($code, 'RedisCache') || str_contains($code, 'remember(')) && !str_contains($code, 'secureCache') && !str_contains($code, 'CacheRegistry')) {
            $extra[] = $this->result('missing_caching_strategy', 'cache', 'Use caching strategy policies for tenant/user scoped cache data.', 'SecureCache adds policy TTLs, tenant/user-aware keys, sensitive-data encryption, tags, invalidation, safe serialization, and stampede protection around existing cache drivers.', 0.92, "\$kernel->secureCache()->remember('student_profile', \$context, fn () => \$value)");
        }
        if ((str_contains($code, 'error_log(') || str_contains($code, 'file_put_contents') || str_contains($code, 'SecurityMonitor') || str_contains($code, 'TamperEvidentAuditLogger') || str_contains($code, 'audit') || str_contains($code, 'log(')) && !str_contains($code, 'monitoringSummary') && !str_contains($code, 'auditIntegrityVerifier') && !str_contains($code, 'logger(')) {
            $extra[] = $this->result('missing_logging_monitoring_engine', 'monitoring', 'Use logging, audit, and monitoring controls for security visibility.', 'The logging and monitoring engine centralizes JSONL log channels, final redaction, audit-chain verification, metrics, alert thresholds, retention cleanup, and monitoring summaries.', 0.93, "\$kernel->logger('security')->warning('Suspicious activity', \$context)");
        }
        if ((str_contains($code, '$_ENV') || str_contains($code, 'getenv(') || str_contains($code, 'APP_KEY') || str_contains($code, 'DATA_KEY') || str_contains($code, 'WEBHOOK_SECRET') || str_contains($code, 'SIGNED_URL_KEY')) && !str_contains($code, 'secretManager') && !str_contains($code, 'SecretManager')) {
            $extra[] = $this->result('missing_secret_management_engine', 'secrets', 'Use environment and secret management for keys and provider credentials.', 'SecretManager centralizes env/provider reads, secret inventory, redaction, purpose-based key derivation, rotation reports, environment validation, and secret scanning.', 0.94, "\$kernel->secretManager()->get('app.key')");
        }
        if ((str_contains($code, 'readForDownload') || str_contains($code, 'Content-Disposition') || str_contains($code, 'download') || str_contains($code, 'storeFromPath') || str_contains($code, 'UploadedFile') || str_contains($code, 'fopen(')) && !str_contains($code, 'protectedDownloadManager') && !str_contains($code, 'FileSecurityRegistry')) {
            $extra[] = $this->result('missing_file_security_engine', 'file_security', 'Use file security policies for uploads, downloads, and documents.', 'FileSecurityRegistry and ProtectedDownloadManager enforce scan status, tenant-aware access, safe download headers, signed URLs, audit logs, checksums, and document/archive inspection hooks.', 0.94, "\$kernel->protectedDownloadManager()->download(\$request, \$file, 'student_document.download')");
        }
        if ((str_contains($code, 'BackupManager') || str_contains($code, 'backup') || str_contains($code, 'restore') || str_contains($code, 'incident') || str_contains($code, 'playbook') || str_contains($code, 'malware_upload_detected')) && !str_contains($code, 'secureBackupManager') && !str_contains($code, 'incidentResponse')) {
            $extra[] = $this->result('missing_backup_incident_engine', 'recovery', 'Use backup, recovery, and incident response controls.', 'The recovery engine adds encrypted/signed backups, manifests, integrity verification, restore dry-runs, retention, incident cases, playbooks, containment actions, evidence collection, and recovery CLI reports.', 0.94, "\$kernel->incidentResponse()->runPlaybook('secret_leak_detected', \$context)");
        }
        if ((str_contains($code, 'vulnerability') || str_contains($code, 'owasp') || str_contains($code, 'cwe') || str_contains($code, 'sql injection') || str_contains($code, 'xss') || str_contains($code, 'ssrf')) && !str_contains($code, 'vulnerabilityMatrix') && !str_contains($code, 'vulnerabilities:report')) {
            $extra[] = $this->result('missing_vulnerability_matrix_engine', 'vulnerability_matrix', 'Use the vulnerability blocking matrix to prove coverage.', 'The matrix maps vulnerabilities to blocking controls, OWASP/CWE IDs, config dependencies, evidence, gaps, scores, and CLI reports.', 0.94, "\$kernel->vulnerabilityCoverageReport()->toArray()");
        }
    ...
    private function defaultRules(): array
    {
        return [
            ...
            ['id' => 'file_security_engine', 'type' => 'file_security', 'title' => 'Use file security policies for protected downloads and documents', 'description' => 'File security controls add scan-gated downloads, tenant-aware policies, safe download headers, signed URLs, checksums, archive inspection hooks, download audit logs, and retention cleanup.', 'keywords' => ['file', 'upload', 'download', 'document', 'attachment', 'content-disposition', 'checksum', 'signed url', 'archive', 'pdf', 'office'], 'example' => "\$kernel->protectedDownloadManager()->download(\$request, \$fileRecord, 'files.download')"],
            ['id' => 'cache_strategy', 'type' => 'cache', 'title' => 'Use caching strategy policies for tenant-aware performance', 'description' => 'SecureCache centralizes TTLs, tenant/user-aware cache keys, sensitive-data encryption, tags/invalidation, safe serialization, and stampede protection on top of file/Redis/database drivers.', 'keywords' => ['cache', 'caching', 'redis', 'filecache', 'databasecache', 'remember', 'tags', 'invalidation', 'stampede', 'ttl'], 'example' => "\$kernel->secureCache()->remember('student_profile', \$context, fn () => \$profile)"],
            ['id' => 'logging_monitoring_engine', 'type' => 'monitoring', 'title' => 'Add logging, audit, and monitoring controls', 'description' => 'Use central log channels, audit integrity verification, metrics, alert rules, and retention cleanup for production observability.', 'keywords' => ['logging', 'audit', 'monitoring', 'metrics', 'alerts', 'incident', 'tamper', 'integrity', 'siem'], 'example' => "\$kernel->monitoringSummary()->toArray()"],
            ['id' => 'backup_incident_engine', 'type' => 'recovery', 'title' => 'Add backup, recovery, and incident response controls', 'description' => 'Use secure backups, manifests, signatures, restore dry-runs, retention cleanup, incident playbooks, containment actions, and evidence reports for operational recovery.', 'keywords' => ['backup', 'restore', 'recovery', 'incident', 'playbook', 'containment', 'evidence', 'disaster', 'drill'], 'example' => "\$kernel->secureBackupManager()->create('daily')"],
